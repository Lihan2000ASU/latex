...
A number of utility status tests are available which can be used by package and
...
\end{decl}
These commands test if the named package, class or file is loaded: this
requires that a matching |\Provides...| line has been given, as described in
Section~\ref{sec:ID}. In the case of \cs{IfFileLoadedTF}, the full
...
\end{decl}
These tests are used to check if a package or class has been loaded with at
...
\textsc{yyyy-mm-dd}. If the package or class is not loaded, the \meta{false}
...
appropriate branch. This will reflect any roll forward or back that has been
applied: the apparent date of the format will be that after roll back.
