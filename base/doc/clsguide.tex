...
\begin{decl}
  |\IfPackageAtLeast(TF)| \arg{package} \arg{date} \arg{true code} \arg{false code}\\
  |\IfClassAtLeast(TF)| \arg{class} \arg{date} \arg{true code} \arg{false code}\\
  |\IfFileAtLeast(TF)| \arg{file} \arg{date} \arg{true code} \arg{false code}
\end{decl}
There are occasions when you need to test if one package if another package has
a particular version (assuming it has been loaded). This can be done using the
commands |\IfPackageAtLeastTF|, etc. Separate commands available to test
packages, classes and general files: in all cases, they rely on the existence
of a |\Provides...| line in the appropriate file being queried. Three versions
...
ISO (|YYYY-MM-DD|) or traditional \LaTeX{} (|YYYY/MM/DD|) formats.
