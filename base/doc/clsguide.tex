...
\subsection{Testing dates of other code}

\begin{decl}
  |\IfPackageAtLeast(TF)| \arg{package-name} \arg{date} \arg{true code} \arg{false code}\\
  |\IfClassAtLeast(TF)| \arg{class-name} \arg{date} \arg{true code} \arg{false code}\\
  |\IfFileAtLeast(TF)| \arg{file-name} \arg{date} \arg{true code} \arg{false code}
\end{decl}
There are occasions when you need to test if a package is
at least a particular date (assuming it has been loaded). This can be done using the
commands |\IfPackageAtLeastTF|, etc. Separate commands are available to test
packages, classes and general files: in all cases, they rely on the existence
of a |\Provides...| line in the file being queried. Three versions
of each command are available, ending |T|, |F| and |TF|, which indicate the
outcome branches required. Thus for example |\IfPakcageAtLeastT| does not need
a \meta{false code} argument. The \meta{date} argument can be given either in
ISO (\textsc{yyyy-mm-dd}) or traditional \LaTeX{} (\textsc{yyyy/mm/dd}) formats.

\begin{decl}
  |\IfFormatAtLeast(TF)| \arg{date} \arg{true code} \arg{false code}
\end{decl}
Tests the release date of the \LaTeX{} format and selects the appropriate
branch. As for the package and class tests described above, |T|, |F| and |TF|
